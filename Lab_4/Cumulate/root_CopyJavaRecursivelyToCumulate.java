        ...
        try (Stream<Path> stream = Files.walk(currentDir)) {
            stream.filter(f -> Files.isRegularFile(f) && f.toString().endsWith(".java"))
                  .forEach(f -> {
                      ...
                      String parentName = (parent != null && !parent.equals(currentDir)) ?
                                            parent.getFileName().toString() : "root";
                      String newFileName = parentName + "_" + f.getFileName().toString();
                      Path destination = cumulateDir.resolve(newFileName);
                      try {
